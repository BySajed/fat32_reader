...
impl Fat32Image {
    pub fn new<P: AsRef<Path>>(path: P) -> io::Result<Self> {
        ...
        // CORRECTION ICI : Nettoyage de l'initialisation de la struct
        let boot_sector = BootSector {
            bytes_per_sector,
            sectors_per_cluster,
            reserved_sector: reserved_sectors, // On mappe la variable (pluriel) vers le champ (singulier)
            number_of_fats,                    // Ajout du champ manquant
            sectors_per_fat,
            root_dir_cluster,
        };
    ...
    pub fn offset_from_cluster(&self, cluster: u32) -> u64 {
        // CORRECTION ICI : Utilisation de .reserved_sector (singulier)
    ...
    // Le reste du code (list_directory, cat_file, etc.) reste inchangé...
